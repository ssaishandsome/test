  ...
  <div id="Content">
    ...
    <div>
    <button @click="toggleCamera">摄像头检测</button>
    <!-- <button @click="sendTestRequest">Send Test Request</button>  -->
    <div v-if="showCamera">
      <img ref="video" :src="videoSrc" class="camera-video" /> <!-- 使用 img 元素来显示视频流 -->
...
export default {
  ...
  methods: {
    ...
    //   console.log('Camera toggled:', this.showCamera);
    ...
    //             this.startVideoFeed(video, canvas); // 传递 video 和 canvas 元素
    //           } else {
    ...
    //   });
    // },
    // async startVideoFeed(video, canvas) {
    ...
    //     console.log('Sending frame to Flask for processing', imageData.slice(0, 30));  // 打印前30个字符
    ...
    //         console.log('Received processed frame from Flask');

    //         const annotatedImage = new Image();
    //         annotatedImage.src = 'data:image/jpeg;base64,' + result.annotatedImage;

    //         context.clearRect(0, 0, canvas.width, canvas.height);
    //         context.drawImage(annotatedImage, 0, 0, canvas.width, canvas.height);
    //       } else {
    ...
    toggleCamera() {
      ...
      if (this.showCamera) {
        this.videoSrc = 'http://localhost:5001/video_feed';
        console.log('Video source set to:', this.videoSrc);
      } else {
        this.videoSrc = '';
      }
    }
